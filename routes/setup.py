...
@bp.route("/test-network", methods=["GET", "POST"])
def test_network():
    """Standalone diagnostic page for the network apply step.

    Renders a form that takes the same inputs as the wizard's Network step
    and runs the same netplan-write + sudo netplan apply + hostnamectl
    sequence, but captures every step's output so you can see exactly where
    it fails. Two submit modes:

      - 'dry-run': writes the YAML to a tempfile, runs `netplan generate`
        against it for validation, but does not apply.
      - 'apply':    same as the wizard's _apply_network.

    Reachable at /setup/test-network in both bootstrap and normal mode; gated
    by the blueprint's before_request hook (admin in normal mode, open in
    bootstrap).
    """
    import shutil
    import subprocess
    ...
    if request.method == "POST":
        ...
        # Build the YAML in memory and show it.
        dns_list = [d for d in dns.split() if d]
        yaml = (
            "# Generated by /setup/test-network (diagnostic page).\n"
            "network:\n"
            "  version: 2\n"
            "  renderer: networkd\n"
            "  ethernets:\n"
        ...
        if dns_list:
            yaml += ("      nameservers:\n        addresses: ["
                     + ", ".join(dns_list) + "]\n")
        step("generated YAML", True, yaml)

        # Validate by writing to a temp dir and running `netplan generate`.
        netplan = shutil.which("netplan")
        if not netplan:
            step("netplan binary", False, "netplan command not found on PATH")
            result = {"steps": steps, "summary": "netplan not installed"}
            return _render("test_network", "setup_test_network.html",
                           form=request.form, result=result, current=_detect_current_network())
        ...
        # Apply path: same logic as _apply_network but with full reporting.
        cfg_path = "/etc/netplan/01-captive-portal.yaml"
        try:
            fd, tmp = _tf.mkstemp(dir=os.path.dirname(cfg_path),
                                  prefix=".test-", suffix=".yaml.tmp")
            with os.fdopen(fd, "w") as f:
                f.write(yaml)
            os.chmod(tmp, 0o600)
            os.replace(tmp, cfg_path)
            step(f"write {cfg_path}", True, f"size={len(yaml)} bytes")
        except Exception as e:
            step(f"write {cfg_path}", False, f"{type(e).__name__}: {e}")
            result = {"steps": steps, "summary": "write failed — aborting apply"}
            return _render("test_network", "setup_test_network.html",
                           form=request.form, result=result, current=_detect_current_network())

        # Run sudo netplan apply
        try:
            p = subprocess.run(["sudo", "-n", "/usr/sbin/netplan", "apply"],
                               capture_output=True, text=True, timeout=30)
            ok = (p.returncode == 0)
            detail = f"rc={p.returncode}\nstdout: {p.stdout.strip()}\nstderr: {p.stderr.strip()}"
            step("sudo netplan apply", ok, detail)
        except Exception as e:
            step("sudo netplan apply", False, f"{type(e).__name__}: {e}")

        # Run sudo hostnamectl set-hostname
        try:
            p = subprocess.run(["sudo", "-n", "/usr/bin/hostnamectl",
                                "set-hostname", hostname],
                               capture_output=True, text=True, timeout=10)
            ok = (p.returncode == 0)
            detail = f"rc={p.returncode}\nstdout: {p.stdout.strip()}\nstderr: {p.stderr.strip()}"
            step("sudo hostnamectl set-hostname", ok, detail)
        except Exception as e:
            step("sudo hostnamectl set-hostname", False, f"{type(e).__name__}: {e}")
        ...
        try:
            ...
            route_out = f"(error: {e})"
        step("post-apply interface state", True, addr_out)
        step("post-apply route table", True, route_out)

        result = {"steps": steps, "summary": "apply complete (see steps above)"}

    return _render("test_network", "setup_test_network.html",
                   form=request.form if request.method == "POST" else None,
                   result=result,
                   current=_detect_current_network())
